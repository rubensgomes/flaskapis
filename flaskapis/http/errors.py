...
from flaskapis.constants import (STATUS_KEY, STATUS_ERROR)
...
class FlaskRESTfulAPI(Api):
    ...
    def pint_bad_request(self, error):
        ...
        if hasattr(error, 'code'):
            code = getattr(error, 'code')
        current_app.logger.debug("Handling a status_code [{0}] error [{1}]"
                                 .format(code, error))
    ...
    def handle_not_found(self, error):
        ...
        current_app.logger.debug("Handling a URL [{0}] not found error [{1}]"
    ...
    def handle_not_acceptable(self, error):
        ...
        current_app.logger.debug("Handling a 406 error [{0}].  "
    ...
    def handle_internal_server_error(self, error):
        ...
        if not error:
            current_app.logger.error("error object empty or null found.")
...
class ErrorResponse:
    ...
    def get_response(code, description, mime_type, title=None):
        ...
        if not code or not isinstance(code, (int)):
            ...
            current_app.logger.warn("error description is blank")
