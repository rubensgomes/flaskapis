"""flaskapis.resources.units subpackage
...
def convert_unit(unit_type, from_unit, from_value, to_unit):
    ...
    if to_unit_dimension != UnitsContainer({"[" + unit_type.name + "]": 1}):
        raise BadRequest(("Parameter to_unit=[{0}] not valid. "
    ...
    if from_unit_dimension != UnitsContainer({"[" + unit_type.name + "]": 1}):
        raise BadRequest("Parameter from_unit=[{0}] not valid. "
                         "A [{1}] unit must be provided."
                        .format(from_unit, unit_type.name))

    from flask import current_app
    current_app.logger.debug("converting [{0} {1}] to [{2}]"
    ...
    current_app.logger.debug("input [{0} {1}] result [{2} {3}]"
                             .format(from_value, from_unit_name, result,
                                     to_unit_name))
